...
When GitHub sync is enabled, GitHub is the operator-visible projection surface for Spec Kitty work: one mission maps to one parent tracking issue, one mission branch, and one mission PR; each WP maps to one managed child issue. Spec Kitty remains the source of truth for mission/WP lanes, worktrees, implementation, review, merge, and acceptance. GitHub issue/PR bodies and controller comments must use the upstream `crnd:*` label contract, status banner shape, structured commentary shape, and final standalone `⟦AI:AUTO-LOOP⟧` sentinel.

GitHub issue comments must not be reduced to status projection only when the controller has substantive reasoning to preserve. Use structured commentary comments for review summaries, solver outputs, meta-judge decisions, scope decisions, evidence summaries, handoffs, and other operator-visible reasoning. Commentary comments must include a TL;DR, structured mission/issue/PR fields, optional score/verdict, detailed evidence when available, the `🤖 controller commentary` marker, and the final standalone sentinel.
...
- If `GITHUB_SYNC_ENABLE=true` (default), promote mission state into GitHub through `github_sync.py`. On mission promotion, ensure or reuse the parent issue. After WP tasks exist, ensure one managed child issue per WP and write `<mission>/consensus-rnd/github-bindings.json`. During WP implement/review dispatch and completion, update the child issue phase label and post a controller status banner. When review, solver, meta-judge, scope, evidence, decision, or handoff reasoning is produced, post a structured commentary comment to the parent or WP issue and persist the markdown body under `<mission>/consensus-rnd/github/`. When a mission branch can be resolved and has commits, open or reuse one mission PR. Resolve the PR base from the final landing branch contract in `plan.md`, `spec.md`, or `consensus-rnd/intake.md`; resolve the PR head from an explicit `--head`, then `meta.target_branch`, then canonical `kitty/*`/`codex/*` mission branch names. Never use a WP lane branch as the mission PR head. The PR body must avoid durable `Closes #<parent>` links; final issue closure is controlled only by `github_sync.py mark-merged` after merge evidence is verified and every bound Spec Kitty WP is in `done`. For legacy/catch-up projection repair only, `bind-merged-pr --merged-pr <number>` may bind an already merged PR without closing issues, and `mark-merged --merged-pr <number>` may bind and close only after `gh pr view` verifies `state=MERGED`, `mergedAt`, and a merge commit oid plus all bound WPs are `done`. These commands must not be used with an unmerged PR, as a substitute for normal mission PR publication, or to close missions whose WPs are only `approved`. Use `--skip-parent` when the parent/source issue is intentionally reused by later missions and only the child issues should close. Default dry-run plans all GitHub operations; `--execute` performs them only after `gh` auth and repo slug preflight succeed.
- When a planning-artifact WP needs final gates before the full Spec Kitty merge, use `spec_backend.py integrate-approved-lanes` to pre-integrate only approved non-planning code lanes into the mission target branch. This command must not edit WP frontmatter, mark WPs done, close issues, delete branches, or clean worktrees; final done transitions and cleanup still belong to `spec-kitty merge`.
...
- Repair legacy/catch-up mission closure after a verified merged PR exists:
  `python3 <skill-root>/scripts/github_sync.py mark-merged --repo "$REPO_ROOT" --mission "<mission-slug>" --merged-pr 93 --execute`
- Bind a verified merged PR without closing issues:
  `python3 <skill-root>/scripts/github_sync.py bind-merged-pr --repo "$REPO_ROOT" --mission "<mission-slug>" --merged-pr 93 --execute`
- Repair child issues while preserving a reused parent/source issue:
  `python3 <skill-root>/scripts/github_sync.py mark-merged --repo "$REPO_ROOT" --mission "<mission-slug>" --merged-pr 93 --skip-parent --execute`
- Plan Spec Kitty handoff:
  `python3 <skill-root>/scripts/spec_backend.py plan --repo "$REPO_ROOT" --title "<title>"`
- Pre-integrate approved code lanes for planning-WP acceptance gates:
  `python3 <skill-root>/scripts/spec_backend.py integrate-approved-lanes --repo "$REPO_ROOT" --mission "<mission-slug>" --target "<target-branch>" --execute`
